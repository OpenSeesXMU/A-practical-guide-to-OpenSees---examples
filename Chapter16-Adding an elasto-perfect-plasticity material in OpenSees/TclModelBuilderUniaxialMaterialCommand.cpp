...
#include "SmearedConcrete.h"

#include "SmearedSteel.h"
...
extern void *OPS_NewPDBondMaterial(void);
extern void *OPS_NewSmearedConcrete(void);

extern void *OPS_NewSmearedSteel(void);
...
*/
	...
	else if ((strcmp(argv[1], "PDBondMaterial") == 0) || (strcmp(argv[1], "PDbondMaterial") == 0)) {       // added by Gu and Sun
	void *theMat = OPS_NewPDBondMaterial();
	if (theMat != 0)
		theMaterial = (UniaxialMaterial *)theMat;
	else
		return TCL_ERROR;
	...
	else if ((strcmp(argv[1], "SmearedConcrete") == 0) || (strcmp(argv[1], "smearedConcrete") == 0)) {       // added by Gu and Sun
	void *theMat = OPS_NewSmearedConcrete();
	if (theMat != 0)
		theMaterial = (UniaxialMaterial *)theMat;
	else
		return TCL_ERROR;
	...
	else if ((strcmp(argv[1], "SmearedSteel") == 0) || (strcmp(argv[1], "smearedSteel") == 0)) {       // added by Gu and Sun
	void *theMat = OPS_NewSmearedSteel();
	if (theMat != 0)
		theMaterial = (UniaxialMaterial *)theMat;
	else
		return TCL_ERROR;
    ...
    */
	...
	if (Tcl_GetDouble(interp,argv[8], &eta) != TCL_OK) {
      ...
      int tag;
      double E, sigmaY, Hkin, Hiso, perturb;
	  Hiso =0.0;
	  perturb = 0.0;
	  if (Tcl_GetInt(interp, argv[2], &tag) != TCL_OK) {
	  ...
	  if (Tcl_GetDouble(interp, argv[3], &E) != TCL_OK) {
	   ...
	   if (Tcl_GetDouble(interp, argv[4], &sigmaY) != TCL_OK) {
		  ...
		  if (argc >= 8) {
		  if (Tcl_GetDouble(interp, argv[7], &perturb) != TCL_OK) {
			  opserr << "WARNING invalid perturb\n";
			  opserr << "uniaxialMaterial UniaxialJ2Plasticity: " << tag << endln;
			  return TCL_ERROR;
		  }
		  }
      ...
      // Parsing was successful, allocate the material
      theMaterial = new UniaxialJ2Plasticity(tag, E, sigmaY, Hkin, Hiso, perturb);       
    }
