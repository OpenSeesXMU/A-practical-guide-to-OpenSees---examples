...
#include <SlideBeamSphere2d.h>
#include <ForceChain2d.h>
#include "bondbasedPeridynamic.h"
#include "BondTruss.h"
#include "StatePDElement3D.h"
#include "StatePDElement2D.h"
#include "weightedCorotTruss.h"
#include <stateBasedPeridynamics.h>
#include <hpd.h>
#include <statePDSoilLiquefaction.h>
#include <statePDDamage.h>
#include <PDNode.h>
extern 
#ifdef _WIN32
int __cdecl
...
extern  void *OPS_bondbasedPeridynamic(void);
extern  void *OPS_BondTrussElement(void);
extern  void *OPS_StatePDElement3D(void);
extern  void *OPS_StatePDElement2D(void);
extern  void *OPS_corotTruss2DElement(void);
extern  void *OPS_stateBasedPeridynamics(void);
extern  void *OPS_HPD(void);
extern  void *OPS_statePDSoilLiquefaction(void);
extern  void *OPS_statePDDamage(void);
extern  void *OPS_modifiedTrussElement(void);
...
//Quan
extern int
TclModelBuilder_addForceChain2d(ClientData, Tcl_Interp *, int, TCL_Char **,
	Domain*, TclModelBuilder *);
...
//Lei
extern int
TclModelBuilder_addSlideBeamSphere2d(ClientData, Tcl_Interp *, int, TCL_Char **,
				Domain*, TclModelBuilder *);
...
//Added by Lei Wang June, 2019
extern int
TclModelBuilder_addbondbasedPeridynamic(ClientData, Tcl_Interp *, int, TCL_Char **,
	Domain*, TclModelBuilder *);

extern int
TclModelBuilder_addstateBasedPeridynamics(ClientData, Tcl_Interp *, int, TCL_Char **,
	Domain*, TclModelBuilder *);

extern int
TclModelBuilder_addHPD(ClientData, Tcl_Interp *, int, TCL_Char **,
	Domain*, TclModelBuilder *);


extern int
TclModelBuilder_addstatePDSoilLiquefaction(ClientData, Tcl_Interp *, int, TCL_Char **,
	Domain*, TclModelBuilder *);


//extern int
//TclModelBuilder_addweightedCorotTruss(ClientData, Tcl_Interp *, int, TCL_Char **,
//	Domain*, TclModelBuilder *);
	...
	Domain *theTclDomain, TclModelBuilder *theTclBuilder)
{
	...
	else if ((strcmp(argv[1], "bondbasedPeridynamic") == 0) || (strcmp(argv[1], "bondbasedPD")) == 0) {
	// ------------------------------added by Lei Wang------------------------------------------
	int result = TclModelBuilder_addbondbasedPeridynamic(clientData, interp, argc, argv,
		theTclDomain, theTclBuilder);
	return result;
	}

	else if ((strcmp(argv[1], "stateBasedPeridynamics") == 0) || (strcmp(argv[1], "stateBasedPD")) == 0) {
	// ------------------------------added by Lei Wang------------------------------------------
	void *theEle;
	if (OPS_GetNDM() == 2 || OPS_GetNDM() == 3)
		theEle = OPS_stateBasedPeridynamics();
	else
	{
		opserr << "TclElementCommand -- unable to create element of type : " << argv[1] << endln;
		return TCL_ERROR;
	...
	//int result = TclModelBuilder_addstateBasedPeridynamics(clientData, interp, argc, argv,
	//	theTclDomain, theTclBuilder);
	//return result;
	}

	else if ((strcmp(argv[1], "HPD") == 0) || (strcmp(argv[1], "hpd")) == 0) {
	// ------------------------------added by Lei Wang------------------------------------------
	void *theEle;
	if (OPS_GetNDM() == 2 || OPS_GetNDM() == 3)
		theEle = OPS_HPD();
	...
	else if ((strcmp(argv[1], "statePDSoilLiquefaction") == 0) || (strcmp(argv[1], "stateBasedPeridynamicsForSoilLiquefaction")) == 0) {
	// ------------------------------added by Lei Wang------------------------------------------
	void *theEle;
	if (OPS_GetNDM() == 2 || OPS_GetNDM() == 3)
		theEle = OPS_statePDSoilLiquefaction();
	else
	{
		opserr << "TclElementCommand -- unable to create element of type : " << argv[1] << endln;
		return TCL_ERROR;
	...
	//int result = TclModelBuilder_addstatePDSoilLiquefaction(clientData, interp, argc, argv,
	//	theTclDomain, theTclBuilder);
	//return result;
	}

	else if ((strcmp(argv[1], "statePDDamage") == 0) || (strcmp(argv[1], "statePDdamage")) == 0) {
	// ------------------------------added by Lei Wang------------------------------------------
	void *theEle;
	if (OPS_GetNDM() == 2 || OPS_GetNDM() == 3)
		theEle = OPS_statePDDamage();
	else
	{
		opserr << "TclElementCommand -- unable to create element of type : " << argv[1] << endln;
		return TCL_ERROR;
	...
	else if ((strcmp(argv[1], "modifiedTruss") == 0) || (strcmp(argv[1], "modifiedTrussElement")) == 0) {
	// ------------------------------added by Lei Wang------------------------------------------
	void *theEle;
	if (OPS_GetNDM() == 2 || OPS_GetNDM() == 3)
		theEle = OPS_modifiedTrussElement();
	else
	{
		opserr << "TclElementCommand -- unable to create element of type : " << argv[1] << endln;
		return TCL_ERROR;
	...
	else if ((strcmp(argv[1], "StatePDElement") == 0) || strcmp(argv[1], "statePDElement") == 0) {
		void *theEle;
		if (OPS_GetNDM() == 2)
			theEle = OPS_StatePDElement2D();
		else if (OPS_GetNDM() == 3)
			theEle = OPS_StatePDElement3D();
		else
		{
			opserr << "TclElementCommand -- unable to create element of type : " << argv[1] << endln;
			return TCL_ERROR;
	...
	void *theEle = OPS_corotTruss2DElement();
	if (theEle != 0)
		theElement = (Element *)theEle;
	...
	else if ((strcmp(argv[1], "bondtruss") == 0) || strcmp(argv[1], "bondTruss") == 0 || (strcmp(argv[1], "BondTruss") == 0)) {

	void *theEle = OPS_BondTrussElement();
	// for backward compatability
	if (theEle == 0) {
	...
	else if ((strcmp(argv[1], "SlideBeamSphere2d") == 0)) {
		// ------------------------------add------------------------------------------
		
		int result = TclModelBuilder_addSlideBeamSphere2d(clientData, interp, argc, argv,
			theTclDomain, theTclBuilder);
		return result;
    } 
	...
	else if (strcmp(argv[1], "forceChain2d") == 0) {
	int result = TclModelBuilder_addForceChain2d(clientData, interp, argc, argv,
		theTclDomain, theTclBuilder);
	return result;
				      ...
				      TclModelBuilder *theTclBuilder)
{
  ...
  //added by Lei Wang, 2018
  //// command: element SlideBeamSphere    int tag, int nd1, int nd2, int nd3,double R1,double R2 , NDMaterial *m;
  
  int
	  TclModelBuilder_addSlideBeamSphere2d(ClientData clientData, Tcl_Interp *interp,
		  int argc,
		  TCL_Char **argv,
		  Domain*theTclDomain,
		  TclModelBuilder *theTclBuilder)
  {
	  ...
	  if (argc != 9) {
		  ...
		  printCommand(argc, argv);
		  opserr << "Want: element SlideBeamSphere eleTag? iNode? jNode? kNode? Rsteel?Rsphere? \n";
		  return TCL_ERROR;
	  }

	  int eleTag, iNode, jNode, kNode, matTag;
	  double R1, R2;
	  ...
	  if (Tcl_GetInt(interp, argv[5], &kNode) != TCL_OK) {
		  opserr << "WARNING invalid kNode\n";
		  opserr << argv[1] << " element: " << eleTag << endln;
		  return TCL_ERROR;
	  }
	  if (Tcl_GetDouble(interp, argv[6], &R1) != TCL_OK) {
		  opserr << "WARNING invalid Rsteel\n";
		  opserr << argv[1] << " element: " << eleTag << endln;
		  return TCL_ERROR;
	  }
	  if (Tcl_GetDouble(interp, argv[7], &R2) != TCL_OK) {
		  opserr << "WARNING invalid Rsphere\n";
		  opserr << argv[1] << " element: " << eleTag << endln;
		  return TCL_ERROR;
	  }
	  if (Tcl_GetInt(interp, argv[8], &matTag) != TCL_OK) {
		  opserr << "WARNING invalid matTag\n";
		  opserr << argv[1] << " element: " << eleTag << endln;
		  return TCL_ERROR;
	  }
		  ...
		  theElement = new SlideBeamSphere2d(eleTag, iNode, jNode, kNode, R1, R2, theMaterial);
  ...
  //////////////////////////
  //// command: element forceChain      &tag      NodeI    NodeJ        SectionTag;

  int
	  TclModelBuilder_addForceChain2d(ClientData clientData, Tcl_Interp *interp,
		  int argc,
		  TCL_Char **argv,
		  Domain*theTclDomain,
		  TclModelBuilder *theTclBuilder)
  {
	  ...
	  if (argc != 6) {
		  opserr << "WARNING incorrect arguments\n";
		  printCommand(argc, argv);
		  opserr << "Want: element forceChain eleTag? iNode? jNode? sectionTag?  \n";
		  return TCL_ERROR;
	  }

	  int eleTag, iNode, jNode, secTag;
	  ...
	  Element *theElement = 0;
	  if (theSection == 0) {  // NDMaterial
		//  opserr << "WARNING section not found\n";
		 // opserr << "Section: " << secTag;
		//  opserr << argv[1] << " element: " << eleTag << endln;
		 // opserr << "2D PlasticityBond2d is used!" << endln;

		  //	??????????????????????
		  sections[0] = 0;
		  NDMaterial *theMaterial = OPS_getNDMaterial(secTag);
		  theElement = new ForceChain2d(eleTag, iNode, jNode, sections, theMaterial, false);


		  // Element *theElement = 0;
	   //	theElement = new ForceChain2d(eleTag, iNode, jNode, sections);
		  ...
		  NDMaterial *theMaterial = 0; //theTclBuilder->getNDMaterial(matID);

		  theElement = new ForceChain2d(eleTag, iNode, jNode, sections, theMaterial, true);
	  }
  ...
  //added by Lei Wang, May, 2019
//// command: element bondbasedPeridynamic    int tag, int ndFirst, int ndLast, double horizon,  uniaxialMaterial *m;

  int
	  TclModelBuilder_addbondbasedPeridynamic(ClientData clientData, Tcl_Interp *interp,
		  int argc,
		  TCL_Char **argv,
		  Domain*theTclDomain,
		  TclModelBuilder *theTclBuilder)
  {
	  ...
	  if (argc != 8) {
		  opserr << "WARNING incorrect arguments\n";
		  printCommand(argc, argv);
		  opserr << "Want: element bondbasedPeridynamic  int tag, int ndm, int ndFirst, int ndLast,  uniaxialMaterial *m; \n";
		  return TCL_ERROR;
	  }

	  int eleTag, iNode, jNode, dimension, matTag;
	  double horizon;
	  ...
	  if (Tcl_GetInt(interp, argv[3], &dimension) != TCL_OK) {
		  opserr << "WARNING invalid iNode\n";
		  opserr << argv[1] << " element: " << iNode << endln;
		  return TCL_ERROR;
	  }


	  if (Tcl_GetInt(interp, argv[4], &iNode) != TCL_OK) {
		  opserr << "WARNING invalid iNode\n";
		  opserr << argv[1] << " element: " << iNode << endln;
		  return TCL_ERROR;
	  }

	  if (Tcl_GetInt(interp, argv[5], &jNode) != TCL_OK) {
		  opserr << "WARNING invalid jNode\n";
		  opserr << argv[1] << " element: " << jNode << endln;
		  return TCL_ERROR;
	  }

	  if (Tcl_GetDouble(interp, argv[6], &horizon) != TCL_OK) {
		  opserr << "WARNING invalid horizon\n";
		  opserr << argv[1] << " element: " << horizon << endln;
		  return TCL_ERROR;
	  }
	  ...
	  if (Tcl_GetInt(interp, argv[7], &matTag) != TCL_OK) {
		  opserr << "WARNING invalid matTag\n";
		  opserr << argv[1] << " element: matTag" << matTag << endln;
		  return TCL_ERROR;
	  }
	 // Node **theNodes;
	  Element *theElement = 0;
	  ...
	  theElement = new bondbasedPeridynamic(eleTag, dimension, iNode, jNode, horizon, *theMaterial);
  ...
  /*
  //add by Lei Wang, Oct 2020, change a method to add the element
  int
	  TclModelBuilder_addstateBasedPeridynamics(ClientData clientData, Tcl_Interp *interp,
		  int argc,
		  TCL_Char **argv,
		  Domain*theTclDomain,
		  TclModelBuilder *theTclBuilder)
  {
	  ...
	  if (numRemainingArgs < 5) {
		  opserr << "WARNING incorrect arguments\n";
		  opserr << "Want: element basedBasedPeridynamic int tag,  int ndFirst, int ndLast, double horizonSize, ; \n";
		  return 0;
	  }
	  ...
	  numData = 1;
	  if (OPS_GetDouble(&numData, &horizon) != 0) {
		  opserr << "WARNING: Invalid horizonSize for the pd element" << iData[0] <<
			  " $iNode $jNode $horizonSize  $r\n";
		  return 0;
	  }
	 
	  int ifZeroMode = 0;
	  double zeroModeG = 1.0;
	  int totalSecondaryNode = 0;
	  int totalFictitousNode = 0;
	  
	  PDNode ** fictitousNodes;
	  fictitousNodes = 0;
	  int count = 0;
	  //optional 
	  while (OPS_GetNumRemainingInputArgs() > 0) {
		  const char* type = OPS_GetString();
		  if (strcmp(type, "-zeroMode") == 0) {
			  if (OPS_GetNumRemainingInputArgs() <= 1) {

				  opserr << "WARNING invalid parameter -zeroMode $ifset $G \n";
				  return 0;

			  }
			  else {

				  if (OPS_GetIntInput(&numData, &ifZeroMode) < 0) {
					  opserr << "WARNING invalid parameter :: if zero mode is considered\n";
					  return 0;
				  }
				  if (OPS_GetDoubleInput(&numData, &zeroModeG) < 0) {
					  opserr << "WARNING invalid zero Mode control parameter G \n";
					  return 0;
				  }
				  count = count + 3;
			  }
		  ...
		  if (strcmp(type, "-secondaryNode") == 0) {
			  if (OPS_GetNumRemainingInputArgs() < 1) {

				  opserr << "WARNING invalid parameter -secondaryNode $totalNum \n";
				  return 0;

			  }
			  else {

				  if (OPS_GetIntInput(&numData, &totalSecondaryNode) < 0) {
					  opserr << "WARNING invalid parameter :: no secondary Node is considered\n";
					  return 0;
				  }
				  count = count + 2;
		  ...
		  if (strcmp(type, "-damageModel") == 0) {
			  if (OPS_GetNumRemainingInputArgs() < 1) {

				  opserr << "WARNING invalid parameter -damageModel $totalNum \n";
				  return 0;

			  }
			  else {

				  if (OPS_GetIntInput(&numData, &ifDam) < 0) {
					  opserr << "WARNING invalid parameter :: no damageModel is considered\n";
					  return 0;
				  }
				  count = count + 2;
		  ...
		  if (strcmp(type, "-fictitousNode") == 0) {
			  if (OPS_GetNumRemainingInputArgs() < 1) {
				  ...
				  if (OPS_GetIntInput(&numData, &totalFictitousNode) < 0) {
					  opserr << "WARNING invalid parameter :: no secondary Node is considered\n";
					  return 0;
				  }
				  int fictitousNodeInfo = 9 + count;
				  TCL_Char **argvfictitousNodes;
				  int  argcfictitousNodes;

				  Tcl_SplitList(interp, argv[fictitousNodeInfo], &argcfictitousNodes, &argvfictitousNodes);

				  int fictitousNodeTag, matTag, num, boundarySide, b1node;
				  double xcoor, ycoor, volumn, zcoor;
				  num = 8;// each fictitousNode has 6 variable
				  NDMaterial* theNDMat;				  
				  if (fictitousNodes == 0) {
					fictitousNodes = new PDNode*[totalFictitousNode];
				  }
				  int ndm = OPS_GetNDM();
				  int ndf = OPS_GetNDF();
				  if (ndm == 2) {
					  for (int i = 0; i < totalFictitousNode; i++) {

						  if (Tcl_GetInt(interp, argvfictitousNodes[num*i + 1], &fictitousNodeTag) != TCL_OK) {
							  opserr << "WARNING fictitousNodes tag xcoor ycoor matTag volumn ";
							  opserr << " : invalid fictitous node tag: " << argvfictitousNodes[1] << endln;
							  Tcl_Free((char *)argvfictitousNodes);
							  return TCL_ERROR;
						  }
						  if (Tcl_GetDouble(interp, argvfictitousNodes[num*i + 2], &xcoor) != TCL_OK) {
							  opserr << "WARNING fictitousNodes tag xcoor ycoor matTag volumn ";
							  opserr << " : invalid fictitous node tag: " << argvfictitousNodes[1] << endln;
							  Tcl_Free((char *)argvfictitousNodes);
							  return TCL_ERROR;
						  }
						  if (Tcl_GetDouble(interp, argvfictitousNodes[num*i + 3], &ycoor) != TCL_OK) {
							  opserr << "WARNING fictitousNodes tag xcoor ycoor matTag volumn ";
							  opserr << " : invalid fictitous node tag: " << argvfictitousNodes[1] << endln;
							  Tcl_Free((char *)argvfictitousNodes);
							  return TCL_ERROR;
						  }
						  if (Tcl_GetInt(interp, argvfictitousNodes[num*i + 4], &matTag) != TCL_OK) {
							  opserr << "WARNING fictitousNodes tag xcoor ycoor matTag volumn ";
							  opserr << " : invalid fictitous node tag: " << argvfictitousNodes[1] << endln;
							  Tcl_Free((char *)argvfictitousNodes);
							  return TCL_ERROR;
						  }
						  if (Tcl_GetDouble(interp, argvfictitousNodes[num*i + 5], &volumn) != TCL_OK) {
							  opserr << "WARNING fictitousNodes tag xcoor ycoor matTag volumn ";
							  opserr << " : invalid fictitous node tag: " << argvfictitousNodes[1] << endln;
							  Tcl_Free((char *)argvfictitousNodes);
							  return TCL_ERROR;
						  ...
						  if (Tcl_GetInt(interp, argvfictitousNodes[num*i + 6], &boundarySide) != TCL_OK) {
							  opserr << "WARNING fictitousNodes tag xcoor ycoor matTag volumn ";
							  opserr << " : invalid fictitous node tag: " << argvfictitousNodes[1] << endln;
							  Tcl_Free((char *)argvfictitousNodes);
							  return TCL_ERROR;
						  }
						  if (Tcl_GetInt(interp, argvfictitousNodes[num*i + 7], &b1node) != TCL_OK) {
							  opserr << "WARNING fictitousNodes tag xcoor ycoor matTag volumn ";
							  opserr << " : invalid fictitous node tag: " << argvfictitousNodes[1] << endln;
							  Tcl_Free((char *)argvfictitousNodes);
							  return TCL_ERROR;
						  ...
						  theNDMat = OPS_getNDMaterial(matTag);
						  theNDMat->getCopy();
						  fictitousNodes[i] = new PDNode(fictitousNodeTag, ndf, xcoor, ycoor, *theNDMat, volumn, boundarySide, b1node);
					  }
				  }
				  else if (ndm == 3) {
					  opserr << "wait to add the code" << endln;				  
				  }
			  } 
		  }
	  }
	  ...
	  theElement = new stateBasedPeridynamics(iData[0], iData[1], iData[2], iData[3], horizon, ifDam, ifZeroMode, zeroModeG, totalSecondaryNode, totalFictitousNode, fictitousNodes);

	  if (theElement == 0) {
		  opserr << "WARNING ran out of memory creating element\n";
		  opserr << argv[1] << " element: " << iData[0] << endln;
		  return TCL_ERROR;
	  }
	 ...
	 // if (fictitousNodes != 0) {
	//	  for (int i = 0; i < totalFictitousNode; i++)
	//		  delete fictitousNodes[i];
	//	  delete[] fictitousNodes;
	 // }
