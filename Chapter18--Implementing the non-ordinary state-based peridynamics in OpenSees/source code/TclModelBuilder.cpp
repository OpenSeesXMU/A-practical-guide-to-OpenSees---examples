...
int
TclCommand_addPDNode(ClientData clientData, Tcl_Interp* interp, int argc,
	TCL_Char** argv);
  ...
  :ModelBuilder(theDomain), ndm(NDM), ndf(NDF), theInterp(interp)
{
  ...
  Tcl_CreateCommand(interp, "pdnode", TclCommand_addPDNode, (ClientData)NULL, NULL);
	...
	TCL_Char** argv)
{
	...
	int matTag;
	double vol;
	UniaxialMaterial* theUniaxialMat;
	NDMaterial* theNDMat;
	if (ndm == 2) {
		if (Tcl_GetDouble(interp, argv[2], &xLoc) != TCL_OK) {
			opserr << "WARNING invalid XCoordinate\n";
		...
		if (Tcl_GetInt(interp, argv[4], &matTag) != 0) {
			opserr << "WARNING: Invalid matTag for PDNode" << nodeId << "\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[5], &vol) != TCL_OK) {
			opserr << "WARNING invalid vol\n";
			opserr << "node: " << nodeId << endln;
			return TCL_ERROR;
		}
		...
		if (Tcl_GetInt(interp, argv[5], &matTag) != 0) {
			opserr << "WARNING: Invalid matTag for PDNode" << nodeId << "\n";
			return TCL_ERROR;
		}
		if (Tcl_GetDouble(interp, argv[6], &vol) != TCL_OK) {
			opserr << "WARNING invalid vol\n";
			opserr << "node: " << nodeId << endln;
			return TCL_ERROR;
		}
	...
	if (ndm == 2)
		thePDNode = new PDNode(nodeId, ndf, xLoc, yLoc, *theNDMat, vol);
	else
		thePDNode = new PDNode(nodeId, ndf, xLoc, yLoc, zLoc, *theNDMat, vol);
	if (theTclDomain->addNode(thePDNode) == false) {    //notice it's addNode, not addPDNode
		opserr << "WARNING failed to add node to the domain\n";
		opserr << "node: " << nodeId << endln;
		delete thePDNode; // otherwise memory leak                                    
		return TCL_ERROR;
	}
	while (currentArg < argc) {
		if (strcmp(argv[currentArg], "-mass") == 0) {
			...
			thePDNode->setDisplayCrds(displayLoc);
		}
		else if (strcmp(argv[currentArg], "-disp") == 0) {
			currentArg++;
			if (argc < currentArg + ndf) {
			...
			thePDNode->setTrialDisp(disp);
			thePDNode->commitState();
		}
		else if (strcmp(argv[currentArg], "-vel") == 0) {
			currentArg++;
			if (argc < currentArg + ndf) {
